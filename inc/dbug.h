...
#define DBUG_RETURN(a1)                                                                  \
    if (1) {                                                                             \
        DBUG_PRINT ("RETURN", ("return value %d=%08x", a1, a1));                         \
        return (DBUG_LEAVE, (a1));                                                       \
    } else                                                                               \
        NOOP
#else
...
#define DBUG_LPRINT(key1, key2, arglist)                                                 \
    {                                                                                    \
        if (_db_on_) {                                                                   \
            _db_lpargs_ (__LINE__, key1, key2);                                          \
            _db_ldoprnt_ arglist;                                                        \
        }                                                                                \
    }
#define DBUG_PRINTE(keyword, arglist) (printf arglist, printf ("\n"), fflush (stdout))
#endif
