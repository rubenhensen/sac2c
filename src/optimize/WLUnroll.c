...
{
    node *letexpr, *cexpr, *tmpn, *bodyn, *argn;
    ids *acc;
    node *partn, *wln;
    ...
    /* create argument list for function application */
    cexpr = NCODE_CEXPR (NPART_CODE (partn));
    tmpn = MakeId (StringCopy (IDS_NAME (acc)), StringCopy (IDS_MOD (acc)), ST_regular);
    ID_VARDEC (tmpn) = IDS_VARDEC (acc);
    argn = MakeExprs (tmpn, MakeExprs (DupTree (cexpr, NULL), NULL));
    ...
    assignn = MakeAssign (MakeLet (letexpr, DupOneIds (acc, NULL)), assignn);
...
ForEachElementHelp (int *l, int *u, int *s, int *w, int dim, int maxdim, node *assignn)
{
    ...
    while (count + act_w < u[dim]) {
        ...
        if (dim + 1 == maxdim) {
            ...
            shpseg = MakeShpseg (
              MakeNums (maxdim, NULL)); /* nums struct is freed inside MakeShpseg. */
...
DoUnrollGenarray (node *wln, node *arg_info)
{
    ...
    elements = 1;
    while (tmpn) {
        elements *= NUM_VAL (EXPRS_EXPR (tmpn));
        tmpn = EXPRS_NEXT (tmpn);
    }

    type
      = TYPES_BASETYPE (IDS_TYPE (LET_IDS (ASSIGN_INSTR (INFO_UNR_ASSIGN (arg_info)))));

    /* drop reshape() */
    /*   args = MakeExprs(args, MakeExprs(MakeNullVec(elements, type), NULL)); */
    /*   let_expr = MakePrf(F_reshape,args); */
    let_expr = MakeNullVec (elements, type);
