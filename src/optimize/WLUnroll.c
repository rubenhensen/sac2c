...
CreateFold (node *assignn, node *index)
{
    ...
    wln = opfunarg[0];    /* (node*) */
    partn = opfunarg[1];  /* (node*) */
    acc = opfunarg[2];    /* (ids*) */
    fundef = opfunarg[3]; /* N_fundef */

    DBUG_ASSERT ((NWITH_TYPE (wln) == WO_foldfun), "WO_foldfun expected!");

#define FOLD_CODE_ADJUSTMENT_WORKS 0
#if FOLD_CODE_ADJUSTMENT_WORKS
    /*
     * get the fold-code
     */
    assigns
      = GetAdjustedFoldCode (NWITH_FUNDEF (wln), acc, NWITH_FUN (wln), NWITH_CEXPR (wln));
     ...
     *              (can be found via 'NWITH_CEXPR( wln)')
     */

    {
        node *fun, *cexpr, *accvar, *funap;

        fun = NWITH_WITHOP (wln);
        cexpr = NWITH_CEXPR (wln);

        accvar
          = MakeId (StringCopy (IDS_NAME (acc)), StringCopy (IDS_MOD (acc)), ST_regular);
...
DoUnrollFold (node *wln, node *arg_info)
{
    ...
    while (partn != NULL) {
        ...
        arg[0] = wln;                                                 /* (node*) */
        arg[1] = partn;                                               /* (node*) */
        arg[2] = LET_IDS (ASSIGN_INSTR (INFO_UNR_ASSIGN (arg_info))); /* (ids*) */
        arg[3] = INFO_UNR_FUNDEF (arg_info);                          /* N_fundef node */
        opfunarg = arg;
        res = ForEachElement (partn, res);
