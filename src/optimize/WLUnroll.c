...
CreateFold (node *assignn, node *index)
{
   ...
   * compilation process.
   * Unfortunately, something goes wrong concerning the 
   * generation / housekeeping of masks 8-((
   * The problems can be exploited by means of the following 
   * example program:
   *
   * int main()
   * {
   *  
   *   z = new_with( [0] <= delta < [1])
   *       fold( +, 0d, 7d);
   *  
   *   return(toi(z));
   * }
  ...
  DBUG_PRINT("OPT", ("Now, we do the dirty trick!\n"));
  ...
  assigns = DoInline(ASSIGN_INSTR( assigns) , funap, arg_info);
