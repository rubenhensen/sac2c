...
sl_def (run_benchmark, void, sl_glparm (struct benchmark *, b))
{
    ...
    if (slr_len (ncores))
        ncores = slr_get (ncores)[0];
    if (slr_len (L))
        L = slr_get (L)[0];
    if (slr_len (results))
        results = slr_get (results)[0];
    if (slr_len (format))
        format = slr_get (format)[0];
    if (slr_len (sep_dump))
        sep_dump = slr_get (sep_dump)[0];
    ...
    for (i = 0; i < L; ++i)
        nsamples += wm[i].wl.current_interval;
    ...
    for (i = 0; i < L; ++i) {
        ...
        for (int j = 0; j < wm[i].wl.current_interval; ++j) {
            ...
            mtperf_report_diffs (wm[i].wl.intervals[j].before,
                                 wm[i].wl.intervals[j].after, report_flags);
        }
    }
    if (!format)
        output_string ("]\n", 2);
