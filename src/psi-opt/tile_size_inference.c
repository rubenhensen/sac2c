...
TSInwith (node *arg_node, node *arg_info)
{
    ...
    INFO_TSI_WLARRAY (arg_info) = INFO_TSI_WLARRAY (old_arg_info);
    /*
...
TSIncode (node *arg_node, node *arg_info)
{
    ...
    if ((INFO_TSI_WOTYPE(arg_info) == WO_genarray) ||
      ...
      INFO_TSI_ACCESS(arg_info) = MakeAccess(ID_VARDEC(INFO_TSI_WLARRAY(arg_info)),
                                             ID_VARDEC(INFO_TSI_INDEXVAR(arg_info)),
                                             ACL_offset,
                                             NULL,
                                             ADIR_write,
...
TSIlet (node *arg_node, node *arg_info)
{
    ...
    if (NODE_TYPE (LET_EXPR (arg_node)) == N_id)
        INFO_TSI_WLARRAY (arg_info) = LET_EXPR (arg_node);
    else
