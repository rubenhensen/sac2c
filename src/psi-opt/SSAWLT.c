...
typedef enum{
  GPT_empty, GPT_full, GPT_partial, GPT_unknown
} gen_prop_t;

#ifndef DBUG_OFF

static
char *gen_prop_str[] = {"GPT_empty", "GPT_full", "GPT_partial", "GPT_unknown"};
#endif
...
static
intern_gen *CutSlices( int *ls, int *us, int *l, int *u, int dim, 
...
static
bool check_genarray_full_part( node *wln)
{
  ...
  DBUG_ASSERT( ((NODE_TYPE( lowern) == N_array) &&
                (NODE_TYPE( uppern) == N_array)),
...
node *CreateFullPartition( node *wln, info *arg_info)
{
  ...
  do_create = ((NWITH_PARTS( wln) < 0) &&
               (GetShapeDim( IDS_TYPE( NWITH_VEC( wln))) >= 0));
  ...
  if (do_create) {
    ...
    coden = MakeNCode( MakeBlock( MakeAssign( MakeLet( coden, _ids),
                                              NULL),
                                  NULL),
                       MakeExprs( idn, NULL));
    ...
    ig = CutSlices( array_null, array_shape, ig->l, ig->u, ig->shape, ig,
                    coden);
...
node * CropBounds( node *wl, shape *max_shp)
{
  ...
  DBUG_ASSERT( ((NWITH_TYPE(wl) == WO_modarray)
                || (NWITH_TYPE(wl) == WO_genarray)),
...
gen_prop_t ComputeGeneratorProperties( node *wl, shape *max_shp)
{
  ...
  DBUG_PRINT( "SSAWLT", ("generator property of with loop in line %d : %s",
                         linenum,
                         gen_prop_str[res]));
    ...
    if (GetShapeDim( IDS_TYPE( _ids)) >= 0) {
      ...
      while (tmpn) {
        if ((N_id == NODE_TYPE( EXPRS_EXPR( tmpn))) &&
            (SSALocateIndexVar( EXPRS_EXPR( tmpn),
                                INFO_SSAWLT_WL( arg_info)) == elts+1)) {
          elts++;
...
node *SSAWLTNwith(node *arg_node, info *arg_info)
{
  ...
  if (NWITH_PARTS( arg_node) == -1) {
    ...
    } else if( (INFO_SSAWLT_GENPROP(arg_info) == GPT_partial) && NWITH_FOLDABLE( arg_node)) {
