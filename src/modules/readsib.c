    ...
    while (tmp != NULL) {
        ...
        if (tmp_done != NULL) {
            ...
            case ST_system:
                 ...
                 *    in this case, first X11.so is searched and if
                 *    that search fails, X11.a is searched for.
                 */
                libtype = "system";
                break;
            default:
            ...
            if (pathname == NULL) {
                ...
                if (*pathname == 0) {
                    /* an empty pathname is generated by CheckSystemLibrary */
                    /* only, so this seems to be such a system library not  */
                    /* in SACs search path. We remove the lib prefix here.  */

                    abspathname = buffer + 3;
                } else {
                    /* generates a fully qualified filename, that begins */
                    /* with a /. This is used to distinguish these two   */
                    /* in cccall.c!                                      */
