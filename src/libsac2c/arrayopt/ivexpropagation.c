...
PropagateNarray (node *arg_node, info *arg_info)
{
...
#ifdef FIXME // oldhat
    if (allex && (!ARRAY_EXTREMAATTACHED (v))) {
#endif //  FIXME // oldhat
        if (allex) {
            /* Build exprs chains of minima and maxima. */
            exprs = ARRAY_AELEMS (v);
            while (exprs != NULL) {
                minv = TBmakeExprs (TBmakeId (AVIS_MINVAL (ID_AVIS (EXPRS_EXPR (exprs)))),
                                    minv);
                maxv = TBmakeExprs (TBmakeId (AVIS_MAXVAL (ID_AVIS (EXPRS_EXPR (exprs)))),
                                    maxv);
                exprs = EXPRS_NEXT (exprs);
            }
            ...
            Iprime = TBmakeId (IVEXIattachExtrema (TBmakeId (minv), TBmakeId (maxv),
                                                   aelemI, &INFO_VARDECS (arg_info),
     ...
     *
     ******************************************************************************/
    static bool isResultHasExtrema (node * arg_node, info * arg_info)
    {
        bool z;
     ...
     *
     ******************************************************************************/
    static node *ExtractedNarrayExtrema (node * arg_node, info * arg_info)
    {
        node *prf;
        node *el0;
        node *z = NULL;
     ...
     *
     ******************************************************************************/
    static bool PrfExtractExtrema (node * arg_node, info * arg_info)
    {
        node *avis;
        bool z = FALSE;
        bool arg1c;
