...
BuildInverseProjections (node *arg_node, info *arg_info)
{
...
#ifdef BROKEN // 2012-06-21
    if ((PMmatchFlatSkipExtremaAndGuards (pat3, ivid))
        || (PMmatchFlatSkipExtremaAndGuards (pat4, ivid))) {
#endif // BROKEN   // 2012-06-21
        if ((PMmatchFlat (pat3, ivid)) || (PMmatchFlat (pat4, ivid))) {
            ...
            for (curpart = 0; curpart < numpart; curpart++) {
                ...
                if ((!AWLFIisHasInverseProjection (
                    ...
                    intrlb = TCgetNthExprsExpr (WLINTERSECTION1 (curpart),
                                                PRF_ARGS (arg_node));
                    intrub = TCgetNthExprsExpr (WLINTERSECTION2 (curpart),
                                                PRF_ARGS (arg_node));
                    ...
                    if ((PMmatchFlat (pat1, intrlb)) && (PMmatchFlat (pat2, intrub))
                        ...
                        && (!AWLFIisIdsMemberPartition (intrub, INFO_CONSUMERWLPART (
                                                                  arg_info)))) {
                        zel
                          = BuildInverseProjectionOne (arg_node, arg_info, arriv, arrlb);
                        zwlb = INFO_ZWITHIDS (arg_info);
                        swaplb = INFO_FINVERSESWAP (arg_info);

                        nlet = TCfilterAssignArg (MatchExpr,
                                                  AVIS_SSAASSIGN (ID_AVIS (intrub)),
                                                  &INFO_PREASSIGNS (arg_info));
                        INFO_PREASSIGNS (arg_info)
                          = TCappendAssign (INFO_PREASSIGNS (arg_info), nlet);

                        zeu
                          = BuildInverseProjectionOne (arg_node, arg_info, arriv, arrub);
                ...
                if ((NULL != zel) && (NULL != zeu)) {
                    ...
                    DBUG_PRINT (
                      "Building axis permute and confluence for %s, partition #%d",
                      AVIS_NAME (ID_AVIS (PRF_ARG1 (arg_node))), curpart);
     ...
     *
     *****************************************************************************/
    node *AWLFIfindWL (node * arg_node)
    {
        node *wl = NULL;
        node *z = NULL;
        pattern *pat;
     ...
     *
     *****************************************************************************/
    node *AWLFIfindWlId (node * arg_node)
    {
        node *wlid = NULL;
        node *wl;
        node *z = NULL;
     ...
     *
     ******************************************************************************/
    static bool noDefaultPartition (node * arg_node)
    {
        node *partn;
        bool z = TRUE;
     ...
     *
     ******************************************************************************/
    node *AWLFItakeDropIv (int takect, int dropct, node *arg_node, node **vardecs,
                           node **preassigns)
    {
        node *z;
        node *arr = NULL;
        ...
        if (takect != SHgetUnrLen (ARRAY_FRAMESHAPE (arr))) {
            ...
            z = DUPdoDupTree (z);
            z = TBmakeArray (TYcopyType (ARRAY_ELEMTYPE (arr)), SHcreateShape (1, takect),
                             z);
    ...
    static node *IntersectBoundsBuilderOne (node * arg_node, info * arg_info,
                                            node * producerPart, int boundnum,
    ...
    static node *IntersectNullComputationBuilder (node * idxmin, node * idxmax,
                                                  node * bound1, node * bound2,
                                                  info * arg_info)
    {
        node *fncall;
        ...
        fncall
          = DSdispatchFunCall (NSgetNamespace ("sacprelude"), "isPartitionIntersectNull",
    ...
    static node *IntersectBoundsBuilder (node * arg_node, info * arg_info, node * ivavis)
    {
        node *expn = NULL;
        node *pwlp;
        node *minarr;
        ...
        while (NULL != pwlp) {
            ...
            curavis
              = IntersectNullComputationBuilder (AVIS_MIN (ivavis), AVIS_MAX (ivavis),
    ...
    static node *attachIntersectCalc (node * arg_node, info * arg_info,
                                      node * ivprimeavis)
    {
        node *ivpavis;
        node *ivassign;
        ...
        if (NULL != intersectcalc) {
            ...
            ivpavis = TBmakeAvis (TRAVtmpVarName (AVIS_NAME (ivprimeavis)),
                                  TYeliminateAKV (ztype));
     ...
     *
     *****************************************************************************/
    bool AWLFIisSingleOpWL (node * arg_node)
    {
        bool z;
     ...
     *
     *****************************************************************************/
    static bool checkProducerWLFoldable (node * arg_node, info * arg_info)
    {
        node *p;
        bool z;
        node *cellavis;
        ...
        if (z) {
            ...
            if (z) {
                DBUG_PRINT (
                  ...
                  AVIS_NAME (ID_AVIS (PRF_ARG2 (arg_node))), WITH_REFERENCED_FOLD (p));
            } else {
     ...
     *
     *****************************************************************************/
    static bool checkConsumerWLFoldable (node * arg_node, info * arg_info)
    {
        bool z;
     ...
     *
     *****************************************************************************/
    static bool checkBothFoldable (node * arg_node, info * arg_info)
    {
     ...
     *
     *****************************************************************************/
    node *AWLFIfundef (node * arg_node, info * arg_info)
    {
        int optctr;
     ...
     *
     *****************************************************************************/
    node *AWLFIassign (node * arg_node, info * arg_info)
    {
        node *let;
        node *oldpreassigns;
     ...
     *
     *****************************************************************************/
    node *AWLFIwith (node * arg_node, info * arg_info)
    {
        info *old_arg_info;
     ...
     *
     *****************************************************************************/
    node *AWLFIpart (node * arg_node, info * arg_info)
    {
        DBUG_ENTER ();
     ...
     *
     *****************************************************************************/
    node *AWLFIids (node * arg_node, info * arg_info)
    {
        DBUG_ENTER ();
     ...
     *
     *****************************************************************************/
    node *AWLFIprf (node * arg_node, info * arg_info)
    {
        node *z;
        node *ivavis = NULL;
