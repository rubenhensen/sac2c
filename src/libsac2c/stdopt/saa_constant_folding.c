...
SAACFprf_take_SxV (node *arg_node, info *arg_info)
{
    ...
    patarg3 = PMvar (1, PMAisVar (&arg1), 0);
    ...
    if ((NULL != shp) && PMmatchFlatSkipExtrema (patarg1, arg_node)
        ...
        /* Skip past any guards on the shape vector */
#ifdef BROKEN
        if (PMmatchFlatSkipGuards (patarg3, shpel)) {
    ...
    node *SAACFprf_drop_SxV (node * arg_node, info * arg_info)
    {
        ...
        /* FIXME The following pattern is likely wrong, and we
         * probably have to chase both shpV and narr back to common ancestor(s).
         * Hence, it is disabled lower down.
         */
        pat2b = PMany (1, PMAgetNode (&shpV), 0);

        /* Case 3 */
        pat3 = PMprf (1, PMAisPrf (F_drop_SxV), 2, PMconst (1, PMAgetVal (&con), 0),
                      PMskip (0));
        ...
        if (PMmatchFlatSkipExtrema (pat1, arg_node)) {
            ...
            DBUG_PRINT ("CF", (" Case 1: drop(shape(V), V) replaced by empty vector"));
        }

        if ((NULL == res) && FALSE /* FIXME See above note re pat2b */
            && (PMmatchFlatSkipExtrema (pat2, arg_node))
            && (PMmatchFlatSkipExtrema (pat2a, shpV))
            && (PMmatchFlatSkipExtrema (pat2b, narr))) {
            /* Case 2: conjure up empty vector. */
            res = TBmakeArray (TYmakeAKS (TYcopyType (
                                            TYgetScalar (ID_NTYPE (PRF_ARG2 (arg_node)))),
                                          SHcreateShape (0)),
                               SHcreateShape (1, 0), NULL);
            DBUG_PRINT ("CF", ("Case 2: drop(shape(V), V)  replaced by empty vector"));
    ...
    node *SAACFprf_non_neg_val_S (node * arg_node, info * arg_info)
    {
        node *res = NULL;
        node *minv;
        constant *con;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_val_lt_shape_SxA (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    static node *saarelat (node * prfarg1, node * prfarg2, info * arg_info, int fna,
                           int fnb, bool minmax, node *prfargres, bool tf, bool recur)
    {
        node *res = NULL;
        pattern *pat;
        constant *arg1c = NULL;
...
#define RELMAX TRUE
        ...
        if (N_id == NODE_TYPE (prfarg1)) {
             ...
             */
            if ((NULL == res) && recur) {
                res = saarelat (prfarg2, prfarg1, arg_info, fnb, fnb, (!minmax),
                                prfargres, tf, FALSE);
            }
             ...
             */
            if ((NULL == res) && recur) {
                res = saarelat (prfarg1, prfarg2, arg_info, fnb, fnb, (!minmax),
                                prfargres, (!tf), FALSE);
            }
     ...
     *
     *****************************************************************************/
    node *SAACFprf_lt_SxS (node * arg_node, info * arg_info)
    {
        node *res = NULL;

        DBUG_ENTER ("SAACFprf_lt_SxS");
        res = saarelat (PRF_ARG1 (arg_node), PRF_ARG2 (arg_node), arg_info, REL_lt,
                        REL_ge, RELMAX, PRF_ARG1 (arg_node), TRUE, TRUE);
     ...
     *
     *****************************************************************************/
    node *SAACFprf_lt_SxV (node * arg_node, info * arg_info)
    {
        node *res = NULL;
        ...
        res = saarelat (PRF_ARG1 (arg_node), PRF_ARG2 (arg_node), arg_info, REL_lt,
                        REL_ge, RELMAX, PRF_ARG2 (arg_node), TRUE, TRUE);
     ...
     *
     *****************************************************************************/
    node *SAACFprf_lt_VxS (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_lt_VxV (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_le_SxS (node * arg_node, info * arg_info)
    {
        node *res = NULL;

        DBUG_ENTER ("SAACFprf_le_SxS");

        res = saarelat (PRF_ARG1 (arg_node), PRF_ARG2 (arg_node), arg_info, REL_le,
                        REL_ge, RELMAX, PRF_ARG1 (arg_node), TRUE, TRUE);
     ...
     *
     *****************************************************************************/
    node *SAACFprf_le_SxV (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_le_VxS (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_le_VxV (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_ge_SxS (node * arg_node, info * arg_info)
    {
        node *res = NULL;

        DBUG_ENTER ("SAACFprf_ge_SxS");

        res = saarelat (PRF_ARG1 (arg_node), PRF_ARG2 (arg_node), arg_info, REL_ge,
                        REL_le, RELMIN, PRF_ARG1 (arg_node), TRUE, TRUE);
     ...
     *
     *****************************************************************************/
    node *SAACFprf_ge_SxV (node * arg_node, info * arg_info)
    {
        node *res = NULL;
        ...
        res = saarelat (PRF_ARG1 (arg_node), PRF_ARG2 (arg_node), arg_info, REL_ge,
                        REL_le, RELMIN, PRF_ARG2 (arg_node), TRUE, TRUE);
     ...
     *
     *****************************************************************************/
    node *SAACFprf_ge_VxS (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_ge_VxV (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_gt_SxS (node * arg_node, info * arg_info)
    {
        node *res = NULL;

        DBUG_ENTER ("SAACFprf_gt_SxS");

        res = saarelat (PRF_ARG1 (arg_node), PRF_ARG2 (arg_node), arg_info, REL_gt,
                        REL_lt, RELMIN, PRF_ARG1 (arg_node), TRUE, TRUE);
     ...
     *
     *****************************************************************************/
    node *SAACFprf_gt_SxV (node * arg_node, info * arg_info)
    {
        node *res = NULL;
        ...
        res = saarelat (PRF_ARG1 (arg_node), PRF_ARG2 (arg_node), arg_info, REL_gt,
                        REL_lt, RELMIN, PRF_ARG2 (arg_node), TRUE, TRUE);
     ...
     *
     *****************************************************************************/
    node *SAACFprf_gt_VxS (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_gt_VxV (node * arg_node, info * arg_info)
    {
        node *res = NULL;
     ...
     *
     *****************************************************************************/
    node *SAACFprf_saabind (node * arg_node, info * arg_info)
    {
        node *res = NULL;
        node *dim;
        node *shp;
