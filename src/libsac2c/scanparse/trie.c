...
trie_add_word (struct trie *trie, const char *word, size_t length, ssize_t info)
{
    ...
    if (child) {
        ...
        if (trie->children_count >= trie->children_size) {
            ...
            trie->children
              = (struct child *)realloc (trie->children,
                                         trie->children_size * sizeof (struct child));
        }
...
/* Searches if a given word can be found in the database and if a trie follows
   it.  Returns trie, which follows given prefix, and sets LAST to true if
   the word itself can be found inside the trie.  */
struct trie *
trie_check_prefix (struct trie *trie, const char *word, size_t length, ssize_t *last)
{
    struct child *child;
...
#define add_word(t, word) trie_add_word (t, word, strlen (word), 1)
    ...
    if (check_search)
        printf ("searching '%s' in the database -- %s\n", argv[1],
                trie_search (t, argv[1], strlen (argv[1])) ? "yes" : "no");
