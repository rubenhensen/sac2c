...
CRTWRPfundef (node *arg_node, info *arg_info)
{
    ...
    /* There should only be one wrapper that is created when doing runtime
     * optimization. Mark that wrapper as indirect.
     */
    if (global.runtime && wrapper != NULL) {
        FUNDEF_ISINDIRECTWRAPPERFUN (wrapper) = TRUE;
    }
