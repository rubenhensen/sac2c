...
CRTWRPfundef (node *arg_node, info *arg_info)
{
     ...
     *   - ALL local fundefs ( but NO guard function generated by type patterns)
    ...
    if (!FUNDEF_ISGUARDFUN (arg_node)) {
        if (!FUNDEF_ISLOCAL (arg_node) && !FUNDEF_WASIMPORTED (arg_node)) {
            ...
            if (FUNDEF_ISWRAPPERFUN (arg_node)) {
                wrapper = FindWrapper (FUNDEF_NS (arg_node),
            ...
            if (wrapper == NULL) {
                ...
                INFO_WRAPPERFUNS (arg_info) =
                    LUTinsertIntoLutS (INFO_WRAPPERFUNS (arg_info),
                                       FUNDEF_NAME (arg_node),
                                       wrapper);
            } else if (dot_args != FUNDEF_HASDOTARGS (wrapper)
                    || dot_rets != FUNDEF_HASDOTRETS (wrapper)) {
                CTIabort (LINE_TO_LOC (global.linenum),
                          "Trying to overload function \"%s\" that expects "
                          "%s %zu argument(s) and %s %zu return value(s) "
                          "with a version that expects %s %zu argument(s) "
            ...
            if (!RefArgMatch (FUNDEF_ARGS (wrapper), FUNDEF_ARGS (arg_node))) {
                CTIabort (NODE_LOCATION (arg_node),
                          "Trying to overload function \"%s\" that expects "
                          "%zu argument(s) and yields %zu return value(s) "
                          "with an instance that expects the same number of "
                          "arguments and yields the same number of return "
                          "values but differs in the number or position "
