...
/**
 * @defgroup rst object and reference parameter restauration
 * @ingroup prec
 *
 ...
 * INFO macros
 */
#define INFO_RSTO_FUNDEF(n) (n->fundef)
#define INFO_RSTO_OBJINITFUNDEF(n) (n->objinitfundef)
#define INFO_RSTO_MODUL(n) (n->modul)
#define INFO_RSTO_RETTYPES(n) (n->rettypes)
#define INFO_RSTO_RHS(n) (n->rhs)
...
MakeInfo ()
{
    ...
    INFO_RSTO_FUNDEF (result) = NULL;
    INFO_RSTO_OBJINITFUNDEF (result) = NULL;
    INFO_RSTO_MODUL (result) = NULL;
    INFO_RSTO_RETTYPES (result) = NULL;
    INFO_RSTO_RHS (result) = NULL;
...
/** <!--******************************************************************-->
 *
 * @fn RSTOdoRestoreObjects
 *
 *  @brief reintroduces reference parameters and references to global
 *         objects again.
 *
 *  @param syntax_tree
 *
 *  @return modified syntax tree containing reference parameters and
 ...
 *
 * function:
 *   char *RSTOobjInitFunctionName( bool before_rename)
 *
 * description:
...
char *
RSTOobjInitFunctionName (bool before_rename)
{
    char *name = "GlobalObjInit";
    char *new_name;

    DBUG_ENTER ("RSTOobjInitFunctionName");
...
CreateObjInitFundef (node *module, info *arg_info)
{
    ...
    fundef = TBmakeFundef (RSTOobjInitFunctionName (TRUE), MODULE_NAME (module), NULL,
    ...
    INFO_RSTO_OBJINITFUNDEF (arg_info) = fundef;
 ...
 *
 * function:
 *   node *RSTOmodule( node *arg_node, info *arg_info)
 *
 * description:
...
node *
RSTOmodule (node *arg_node, info *arg_info)
{
    DBUG_ENTER ("RSTOmodule");
 ...
 *
 * Function:
 *   node *RSTOobjdef( node *arg_node, info *arg_info)
 *
 * Description:
 *
 *
...
node *
RSTOobjdef (node *arg_node, info *arg_info)
{
    DBUG_ENTER ("RSTOobjdef");
    ...
    /* insert init code */
    FUNDEF_BODY (INFO_RSTO_OBJINITFUNDEF (arg_info))
      = InsertObjInit (FUNDEF_BODY (INFO_RSTO_OBJINITFUNDEF (arg_info)), arg_node);
 ...
 *
 * function:
 *   node *RSTOfundef(node *arg_node, info *arg_info)
 *
 * description:
 *
 *
...
node *
RSTOfundef (node *arg_node, info *arg_info)
{
    DBUG_ENTER ("RSTOfundef");
     ...
     */
    INFO_RSTO_RETTYPES (arg_info) = FUNDEF_TYPES (arg_node);
    FUNDEF_RETS (arg_node) = TRAVdo (FUNDEF_RETS (arg_node), arg_info);
    FUNDEF_TYPES (arg_node) = INFO_RSTO_RETTYPES (arg_info);
 ...
 *
 * function:
 *   node *RSTOarg( node *arg_node, info *arg_info)
 *
 * description:
...
node *
RSTOarg (node *arg_node, info *arg_info)
{
    DBUG_ENTER ("RSTOarg");
 ...
 *
 * Function:
 *   node *RSTOvardec( node *arg_node, info *arg_info)
 *
 * Description:
...
node *
RSTOvardec (node *arg_node, info *arg_info)
{
    DBUG_ENTER ("RSTOvardec");
 ...
 *
 * function:
 *   node *RSTOassign( node *arg_node, info *arg_info)
 *
 * description:
 *
 *
...
node *
RSTOassign (node *arg_node, info *arg_info)
{
    DBUG_ENTER ("RSTOassign");
 ...
 *
 * function:
 *   node *RSTOlet( node *arg_node, info *arg_info)
 *
 * description:
 ...
 *****************************************************************************/
node *
RSTOlet (node *arg_node, info *arg_info)
{
    DBUG_ENTER ("RSTOlet");

    if (LET_IDS (arg_node) != NULL) {
        INFO_RSTO_RHS (arg_info) = LET_EXPR (arg_node);
 ...
 *
 * Function:
 *   node *RSTOexprs_ap( node *current, node *formal)
 *
 * Description:
 ...
 *****************************************************************************/
static node *
RSTOexprs_ap (node *current, node *formal)
{
    node *expr;

    DBUG_ENTER ("RSTOexprs_ap");

    if (formal != NULL) {
        EXPRS_NEXT (current) = RSTOexprs_ap (EXPRS_NEXT (current), ARG_NEXT (formal));
 ...
 *
 * Function:
 *   node *RSTOexprs_return( node *ret_exprs, node *ret_node)
 *
 * Description:
...
static node *
RSTOexprs_return (node *ret_exprs, node *return_node, node *formal)
{
    node *tmp;

    DBUG_ENTER ("RSTOexprs_return");

    if (formal != NULL) {
        if (EXPRS_NEXT (ret_exprs) != NULL) {
            EXPRS_NEXT (ret_exprs)
              = RSTOexprs_return (EXPRS_NEXT (ret_exprs), return_node, RET_NEXT (formal));
        }
 ...
 *
 * Function:
 *   node *RSTOap( node *arg_node, info *arg_info)
 *
 * Description:
 *   Traverses the current arguments using function RSTOexprs_ap that is given
 *   a pointer to the first formal parameter of the applied function.
 *
 ******************************************************************************/

node *
RSTOap (node *arg_node, info *arg_info)
{
    node *arg;
    node *ret_node;
    ...
    if (FUNDEF_STATUS (AP_FUNDEF (arg_node)) == ST_classfun) {
        ...
        if (AP_ARGS (arg_node) != NULL) {
            AP_ARGS (arg_node)
              = RSTOexprs_ap (AP_ARGS (arg_node), FUNDEF_ARGS (AP_FUNDEF (arg_node)));
        }
    }
 ...
 *
 * Function:
 *   node *RSTOreturn( node *arg_node, info *arg_info)
 *
 * Description:
 *   Traverses the return values using function RSTOexprs_return.
 *
 ******************************************************************************/

node *
RSTOreturn (node *arg_node, info *arg_info)
{
    DBUG_ENTER ("RSTOreturn");

    if (RETURN_EXPRS (arg_node) != NULL) {
        RETURN_EXPRS (arg_node) = RSTOexprs_return (RETURN_EXPRS (arg_node), arg_node);
    }
 ...
 *
 * Function:
 *   node *RSTOid( node *arg_node, info *arg_info)
 *
 * Description:
 *
 *
...
node *
RSTOid (node *arg_node, info *arg_info)
{
    DBUG_ENTER ("RSTOid");
 ...
 *
 * Function:
 *   node *RSTOids( node *arg_node, info *arg_info)
 *
 * Description:
...
node *
RSTOids (node *arg_node, info *arg_info)
{
    bool found;
    node *exprs;
    node *rhs;
     ...
     *
     * Function:
     *   node *RSTOret( node *arg_node, info *arg_info)
     *
     * Description:
     *
     *
    ...
    node *RSTOret (node * arg_node, info * arg_info)
    {
        ...
        tmptypes = INFO_RSTO_RETTYPES (arg_info);
        INFO_RSTO_RETTYPES (arg_info) = TYPES_NEXT (tmptypes);

        if (RET_NEXT (arg_node) != NULL) {
            RET_NEXT (arg_node) = TRAVdo (RET_NEXT (arg_node), arg_info);
        }

        TYPES_NEXT (tmptypes) = INFO_RSTO_RETTYPES (arg_info);
        ...
        INFO_RSTO_RETTYPES (arg_info) = tmptypes;
