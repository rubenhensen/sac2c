...
PRECSPMD (node *arg_node, node *arg_info)
{
    ...
    /*
     * remove vardec of local vars from FUNDEF_VARDEC
     */

    spmd_ids = SPMD_LOCAL (arg_node);
    while (spmd_ids != NULL) {
        DBUG_ASSERT ((IDS_VARDEC (spmd_ids) != NULL), "vardec of ids not found");
        vardecs = FUNDEF_VARDEC (INFO_PREC_FUNDEF (arg_info));
        DBUG_ASSERT ((vardecs != NULL), "vardec for local var not found");

        if (IDS_VARDEC (spmd_ids) == vardecs) {
            /*
             * first vardec is local
             *  -> move FUNDEF_VARDEC to next vardec
             */
            vardecs = FUNDEF_VARDEC (INFO_PREC_FUNDEF (arg_info))
              = VARDEC_NEXT (FUNDEF_VARDEC (INFO_PREC_FUNDEF (arg_info)));
        } else {
            /* search for local vardec in inner of chain */
            while (IDS_VARDEC (spmd_ids) != VARDEC_NEXT (vardecs)) {
                DBUG_ASSERT ((VARDEC_NEXT (vardecs) != NULL),
                             "vardec for local var not found");
                vardecs = VARDEC_NEXT (vardecs);
            }
            /*
             * we have found the vardec for the local var
             *  -> remove it from the vardec-chain 'FUNDEF_VARDEC'
             */
            VARDEC_NEXT (vardecs) = VARDEC_NEXT (VARDEC_NEXT (vardecs));
        }
    ...
    /*
     * insert vardecs of local vars into SPMD_VARDEC
     */
