 ...
 *
 * function:
 *   node *MakeAllocArrayICMs( ids *ids_chain, node *next)
 *
 * description:
...
static node *
MakeAllocArrayICMs (ids *ids_chain, node *next)
{
    simpletype s_type;
    node *assign;
    ...
    while (ids_chain != NULL) {
        if (IDS_REFCNT (ids_chain) >= 0) {
            GET_BASIC_SIMPLETYPE (s_type, IDS_TYPE (ids_chain));
            assign = MakeAssignIcm3 ("ND_ALLOC_ARRAY", MakeId1 (type_string[s_type]),
                                     MakeId2 (DupOneIds (ids_chain, NULL)),
 ...
 *
 * function:
 *   node *MakeAllocArrayICMs_reuse( ids *ids_chain, node *next)
 *
 * description:
...
static node *
MakeAllocArrayICMs_reuse (ids *ids_chain, node *next)
{
    simpletype s_type;
    node *assign;
    node *last_assign = NULL;
        ...
        if (IDS_REFCNT (ids_chain) >= 0) {
            ...
            assign
              = MakeAssign (MakeIcm3 ("ND_ALLOC_ARRAY", MakeId1 (type_string[s_type]),
                                      MakeId2 (DupOneIds (ids_chain, NULL)), MakeNum (0)),
 ...
 *
 * function:
 *   node *MakeIncRcICM( char *name, int rc, int num)
 *
 * description:
...
static node *
MakeIncRcICM (char *name, int rc, int num)
{
    node *icm = NULL;
    ...
    if (rc >= 0) { /* is it a refcounted data? */
        ...
        if (num > 0) {
            icm
              = MakeAssignIcm2 ("ND_INC_RC", MakeId1 (StringCopy (name)), MakeNum (num));
        }
    }
    ...
    if (rc >= 0) { /* is it a refcounted data? */
        ...
        if (num > 0) {
            if (rc - num > 0) { /* definitely no FREE needed? */
                icm = MakeAssignIcm2 ("ND_DEC_RC", MakeId1 (StringCopy (name)),
                                      MakeNum (num));
            } else {
                if (IsNonUniqueHidden (type)) {
                    icm = MakeAssignIcm3 ("ND_DEC_RC_FREE_HIDDEN",
                                          MakeId1 (StringCopy (name)), MakeNum (num),
                                          MakeId1 (GenericFun (1, type)));
                } else {
                    icm = MakeAssignIcm2 ("ND_DEC_RC_FREE_ARRAY",
                                          MakeId1 (StringCopy (name)), MakeNum (num));
                }
            }
        }
...
Ids2IncRcICMs (ids *ids_chain, node *next)
{
    ...
    while (ids_chain != NULL) {
        assign = MakeIncRcICM (IDS_NAME (ids_chain), IDS_REFCNT (ids_chain),
                               IDS_REFCNT (ids_chain));
...
COMPFundef (node *arg_node, node *arg_info)
{
    ...
    if ((rettypes != NULL) && (TYPES_BASETYPE (rettypes) != T_void)
        ...
        return_node = EXPRS_NEXT (ICM_ARGS (return_icm));
    } else {
        return_node = NULL;
    }
    ...
    while ((rettypes != NULL) && (TYPES_BASETYPE (rettypes) != T_void)
        ...
        if (NULL == FUNDEF_BODY (arg_node)) {
            ...
            icm_args = MakeExprs (MakeId1 (""), NULL);
        } else {
            /*
        ...
        if ((MUST_REFCOUNT (TYPE, rettypes))
            ...
            icm_args = MakeExprs (MakeId1 ("out"), icm_args);
        }
    ...
    if ((rettypes != NULL) && (TYPES_BASETYPE (rettypes) == T_dots)) {
        ...
        icm_args = MakeExprs (MakeId1 ("in"), icm_args);
...
COMPPrf (node *arg_node, node *arg_info)
{
    ...
    if (PRF_PRF (arg_node) > F_neq) {
         ...
         * CAUTION: The RC of the new variable must be set accordingly!!!
         * Example:
         *     A:n = prf( A:1, ...);
         *     ... A:n ... A:1 ...    // n references of 'A'
         *   ist transformed into
         *     __A:1 = A:1;
         *     A:n = prf( __A:1, ...);
         *     ... A:n ... A:1 ...
         */
        while (NULL != exprs) {
            if (N_id == NODE_TYPE (EXPRS_EXPR (exprs))) {
                if (0 == strcmp (IDS_NAME (let_ids), ID_NAME (EXPRS_EXPR (exprs)))) {
                    if (0 == insert_assign) {
                        new_id = MakeId1 (TmpVarName (IDS_NAME (let_ids)));
                        ID_REFCNT (new_id) = 1; /* very important!! */
                        old_id = MakeId3 (let_ids);
                        CREATE_2_ARY_ICM (new_assign, "ND_KS_ASSIGN_ARRAY", old_id,
                                          new_id);
                        ...
                        INFO_COMP_VARDECS (arg_info)
                          = AddVardec (INFO_COMP_VARDECS (arg_info),
                                       VARDEC_TYPE (IDS_VARDEC (let_ids)), new_id,
                                       INFO_COMP_FUNDEF (arg_info));
                    }

                    /* now rename N_id */
                    EXPRS_EXPR (exprs) = FreeTree (EXPRS_EXPR (exprs));
                    EXPRS_EXPR (exprs) = DupNode (new_id);
                }
            }
        ...
        case F_reshape: {
            ...
            if (0 == strcmp (ID_NAME (arg2), IDS_NAME (let_ids))) {
                /*
                ...
                icms = MakeAssignIcm2 ("ND_KS_ASSIGN_ARRAY", DupTree (arg2),
                                       MakeId3 (DupOneIds (let_ids, NULL)));
                ASSIGN_NEXT (icms)
...
COMPArray (node *arg_node, node *arg_info)
{
   ...
   /* compute basic_type of result */
   GET_BASIC_SIMPLETYPE(s_type, VARDEC_TYPE(IDS_VARDEC(INFO_COMP_LASTIDS(arg_info))));
   ...
   if( elems_are_non_hidden_scalars) {
     icm2 = MakeIcm3( "ND_CREATE_CONST_ARRAY_S",
                      name_id_node, 
                      MakeNum( n_elems), 
                      elems);
   }
   else {
     icm2 = MakeIcm3( "ND_CREATE_CONST_ARRAY_H",
                      name_id_node, 
                      MakeNum( n_elems), 
                      elems);
   }
   ...
   DBUG_RETURN(res);
    ...
    {
        node *tmp, *next, *exprs, *icm_arg, *save_icm_arg, *id_node, *tag_node,
          *next_assign, *first_assign, *add_assigns_before, *fundef_args,
          *add_assigns_after, *last_assign, **icm_tab, *icm_tab_entry;
        ids *ids;
        ...
        while (ids != NULL) {
            ...
            if (MUST_REFCOUNT (IDS, ids)) {
                if (FUN_DOES_REFCOUNT (AP_FUNDEF (arg_node), cnt_param)) {
                    ...
                    next_assign
                      = MakeAdjustRcICM (IDS_NAME (ids), IDS_TYPE (ids), IDS_REFCNT (ids),
                                         IDS_REFCNT (ids) - 1);
                    if (next_assign != NULL) {
                        APPEND_ASSIGNS (first_assign, next_assign);
                    }
                ...
                if (FUN_DOES_REFCOUNT (AP_FUNDEF (arg_node), cnt_param)) {
                    ...
                    if (ID_REFCNT (EXPRS_EXPR (exprs)) > 1) {
                        ...
                        CREATE_3_ARY_ICM (next_assign, "ND_DEC_RC_FREE_HIDDEN",
                                          MakeId1 (ID_NAME (EXPRS_EXPR (exprs))),
                                          MakeNum (1),
                                          MakeId1 (GenericFun (1, ID_TYPE (EXPRS_EXPR (
                                                                    exprs)))));
                    } else {
                        CREATE_2_ARY_ICM (next_assign, "ND_DEC_RC_FREE_ARRAY",
                                          MakeId1 (ID_NAME (EXPRS_EXPR (exprs))),
    ...
    node *COMPNormalFunReturn (node * arg_node, node * arg_info)
    {
         ...
         */
        icm_args = MakeExprs (MakeId1 (""), MakeExprs (MakeNum (0), NULL));
        ...
        while (ret_expr != NULL) {
            ...
            while (tmp != ret_expr) {
                ...
                if (!strcmp (ID_NAME (EXPRS_EXPR (ret_expr)),
                             ID_NAME (EXPRS_EXPR (tmp)))) {
                    /*
            ...
            if (rename) {
                last_icm_arg = EXPRS_NEXT (last_icm_arg)
                  = MakeExprs (MakeId1 (TmpVarName (ID_NAME (EXPRS_EXPR (ret_expr)))),
                               NULL);
            } else {
                last_icm_arg = EXPRS_NEXT (last_icm_arg)
                  = MakeExprs (DupNode (EXPRS_EXPR (ret_expr)), NULL);
    ...
    node *COMPLoop (node * arg_node, node * arg_info)
    {
        ...
        if (N_do == NODE_TYPE (arg_node)) {
            label = MakeId1 (TmpVarName (LABEL_NAME));
    ...
    {
        node *icm_arg, *icm_args, *type_id_node, *id_node;
        char *icm_name;
        int i;
        types *full_type;
        ...
        if (IsArray (OBJDEF_TYPE (arg_node))) {
            ...
            if ((OBJDEF_STATUS (arg_node) == ST_imported_mod)
                ...
                icm_name = "ND_KD_DECL_EXTERN_ARRAY";
            } else {
                icm_name = "ND_KS_DECL_GLOBAL_ARRAY";
            ...
            OBJDEF_ICM (arg_node) = MakeIcm (icm_name, icm_args);
        } else {
    ...
    node *COMPSpmd (node * arg_node, node * arg_info)
    {
        ...
        icm_args
          = BuildParamsByDFM (SPMD_SHARED (arg_node), "shared", &num_args, icm_args);
        ...
        assigns
          = AppendAssign (BLOCK_SPMD_PROLOG_ICMS (FUNDEF_BODY (SPMD_FUNDEF (arg_node))),
                          assigns);
    ...
    node *COMPSync (node * arg_node, node * arg_info)
    {
            ...
            while (assign != NULL) {
                ...
                if (NODE_TYPE (with) == N_Nwith2) {
                    ...
                    if ((NWITH2_TYPE (with) == WO_foldprf)
                         ...
                         * <fold_type>, <accu_var>
                         */
                        fold_args = MakeExprs (MakeId1 (fold_type),
                                               MakeExprs (MakeId1 (IDS_NAME (with_ids)),
                                                          fold_args));

                        DBUG_PRINT ("COMPi", ("last's folds %s is %s",
        ...
        last_assign = NULL;
        /*
         * prolog == TRUE: current assignment is in front of WL_..._BEGIN-ICM (part of
         * prolog) epilog == TRUE: current assignment is behind WL_..._END-ICM (part of
         * epilog)
         */
        prolog = TRUE;
        epilog = FALSE;
        ...
        while (assign != NULL) {
            ...
            if (NODE_TYPE (instr) == N_icm) {
                ...
                if (var_name != NULL) {
                     ...
                     * fold-results (out-variables). Also outrep-varibale-icm's are not
                     * touched.
                     */
                    if (DFMTestMaskEntry (SYNC_IN (arg_node), var_name, NULL)
                        ...
                        DBUG_ASSERT ((prolog || epilog),
                                     ("icm has to be prolog or epilog"));
                        DBUG_ASSERT ((!(prolog && epilog)),
                                     ("icm cannot be in prolog and epilog at the same "
                                      "time"));
                        if (prolog) {
        ...
        if (!SYNC_FIRST (arg_node)) {
            ...
            assigns
              = AppendAssignIcm (assigns, "MT_MASTER_END", MakeExprsNum (barrier_id));
            assigns
              = AppendAssignIcm (assigns, "MT_WORKER_BEGIN", MakeExprsNum (barrier_id));
            ...
            assigns = AppendAssignIcm (assigns, "MT_MASTER_RECEIVE_SYNCARGS",
                                       DupTree (sync_args));
            assigns
              = AppendAssignIcm (assigns, "MT_WORKER_END", MakeExprsNum (barrier_id));
            ...
            BLOCK_SPMD_PROLOG_ICMS (FUNDEF_BODY (INFO_COMP_FUNDEF (arg_info)))
              = prolog_icms;
        ...
        if (DFMTestMask (SYNC_INOUT (arg_node)) > 0) {
            if (DFMTestMask (SYNC_OUT (arg_node)) > 0) {
                if (DFMTestMask (SYNC_OUT (arg_node)) > 1) {
                    ...
                    /* possible, but not implemented: icm_name =
                     * "MT_SYNC_ONEFOLD_NONFOLD"; */
                    barrier_args = MakeExprs (MakeNum (num_barrier_args), barrier_args);
                    barrier_args = MakeExprs (MakeNum (barrier_id), barrier_args);
                    icm_name = "MT_SYNC_FOLD";
        ...
        if (!SYNC_LAST (arg_node)) {
            assigns
              = AppendAssignIcm (assigns, "MT_MASTER_BEGIN", MakeExprsNum (barrier_id));
        } else {
            assigns
    ...
    node *COMPNwith2 (node * arg_node, node * arg_info)
    {
        ...
        if ((NWITH2_TYPE (arg_node) == WO_genarray)
            ...
            if (fold_vardecs != NULL) {
                 ...
                 */
                FUNDEF_VARDEC (fundef)
                  = AppendVardecs (FUNDEF_VARDEC (fundef), fold_vardecs);
         ...
         */
        assigns
          = AppendAssign (assigns, Ids2DecRcICMs (NWITH2_DEC_RC_IDS (arg_node), NULL));
    ...
    node *COMPWLseg (node * arg_node, node * arg_info)
    {
        ...
        if ((NWITHOP_TYPE (NWITH2_WITHOP (wl_node)) == WO_genarray)
            ...
            assigns = MakeAssign (MakeIcm4 ("WL_INIT_OFFSET",
                                            MakeNum (TYPES_DIM (
                                              VARDEC_OR_ARG_TYPE (IDS_VARDEC (wl_ids)))),
                                            MakeId1 (IDS_NAME (wl_ids)),
                                            MakeId1 (IDS_NAME (
                                              NWITHID_VEC (NWITH2_WITHID (wl_node)))),
                     ...
                     MakeExprs (MakeId2 (DupOneIds (ids_scalar, NULL)),
                                MakeExprs (MakeNum (WLBLOCK_BOUND1 (arg_node)),
                                           MakeExprs (MakeNum (WLBLOCK_BOUND2 (arg_node)),
                                                      MakeExprs (MakeNum (WLBLOCK_STEP (
                                                                   arg_node)),
                                                                 NULL))))));
                     ...
                     MakeExprs (MakeId2 (DupOneIds (ids_scalar, NULL)),
                                MakeExprs (MakeNum (WLUBLOCK_BOUND1 (arg_node)),
                                           MakeExprs (MakeNum (
                                                      ...
                                                      MakeExprs (MakeNum (WLUBLOCK_STEP (
                                                                   arg_node)),
                                                                 NULL))))));
                     ...
                     MakeExprs (MakeId2 (DupOneIds (ids_scalar, NULL)),
                                MakeExprs (MakeNum (WLSTRIDE_BOUND1 (arg_node)),
                                           MakeExprs (MakeNum (
                                                      ...
                                                      MakeExprs (MakeNum (WLSTRIDE_STEP (
                                                                   arg_node)),
                                                                 NULL))))));
    ...
    node *COMPWLgrid (node * arg_node, node * arg_info)
    {
        ...
        if (WLGRID_NEXTDIM (arg_node) != NULL) {
            ...
            if ((WLGRID_CODE (arg_node) != NULL)
                ...
                DBUG_ASSERT ((NCODE_CBLOCK_INSTR (WLGRID_CODE (arg_node)) != NULL),
                             "first instruction of block is NULL (should be a N_empty "
                             "node)");
            ...
            if (icm_name != NULL) {
                assigns
                  = AppendAssign (assigns, MakeAssign (MakeIcm1 (icm_name, icm_args2),
                                                       dec_rc_cexpr));
            }
        }
        ...
        assigns
          = AppendAssign (assigns, MakeAssignIcm1 (icm_name_end, DupTree (icm_args)));
    ...
    node *COMPWLsegVar (node * arg_node, node * arg_info)
    {
         ...
         */
        assigns = AppendAssign (assigns,
                                MakeAssign (SCHCompileSchedulingEnd (WLSEGVAR_SCHEDULING (
                                                                       arg_node),
                                                                     arg_node),
                                            NULL));
        assigns = MakeAssign (SCHCompileSchedulingBegin (WLSEGVAR_SCHEDULING (arg_node),
                                                         arg_node),
                              assigns);
    ...
    node *COMPWLgridVar (node * arg_node, node * arg_info)
    {
        ...
        if (WLGRIDVAR_NEXTDIM (arg_node) != NULL) {
            ...
            if (WLGRIDVAR_CODE (arg_node) != NULL) {
                ...
                if (NODE_TYPE (NCODE_CBLOCK_INSTR (WLGRIDVAR_CODE (arg_node)))
                    != N_empty) {
    ...
    node *MakeAssign_ (node * instr, node * next)
    {
        node *result;
    ...
    node *MakeAssigns1 (node * part1)
    {
        return (MakeAssign_ (part1, NULL));
    }

    node *MakeAssigns2 (node * part1, node * part2)
    {
        return (MakeAssign_ (part1, MakeAssigns1 (part2)));
    }

    node *MakeAssigns3 (node * part1, node * part2, node * part3)
    {
        return (MakeAssign_ (part1, MakeAssigns2 (part2, part3)));
    }

    node *MakeAssigns4 (node * part1, node * part2, node * part3, node * part4)
    {
        return (MakeAssign_ (part1, MakeAssigns3 (part2, part3, part4)));
    }

    node *MakeAssigns5 (node * part1, node * part2, node * part3, node * part4,
                        node * part5)
    {
        return (MakeAssign_ (part1, MakeAssigns4 (part2, part3, part4, part5)));
    }

    node *MakeAssigns6 (node * part1, node * part2, node * part3, node * part4,
                        node * part5, node * part6)
    {
        return (MakeAssign_ (part1, MakeAssigns5 (part2, part3, part4, part5, part6)));
    }

    node *MakeAssigns7 (node * part1, node * part2, node * part3, node * part4,
                        node * part5, node * part6, node * part7)
    {
        return (
          MakeAssign_ (part1, MakeAssigns6 (part2, part3, part4, part5, part6, part7)));
    }

    node *MakeAssigns8 (node * part1, node * part2, node * part3, node * part4,
                        node * part5, node * part6, node * part7, node * part8)
    {
        return (MakeAssign_ (part1, MakeAssigns7 (part2, part3, part4, part5, part6,
                                                  part7, part8)));
    }

    node *MakeAssigns9 (node * part1, node * part2, node * part3, node * part4,
                        node * part5, node * part6, node * part7, node * part8,
                        node * part9)
    {
        return (MakeAssign_ (part1, MakeAssigns8 (part2, part3, part4, part5, part6,
                                                  part7, part8, part9)));
    }

    /* #### */
    node *MakeAllocs (DFMmask_t mask)
    {
        char *name;
        node *result;
        node *icm;
...
#define DO_NOT_COMPILE_MTN_xxx
    ...
    node *COMPMt (node * arg_node, node * arg_info)
    {
        ...
        /*
          allocate  = MakeIcm2( "MT2_ALLOCATE",
                                ...
                                BuildParamsByDFM( MT_ALLOC( arg_node), "alloc", NULL,
          NULL));
    ...
    node *COMPSt (node * arg_node, node * arg_info)
    {
        ...
        DBUG_PRINT ("COMPjhs",
                    ("compiling %s attrib: %s status: %s", FUNDEF_NAME (fundef),
        ...
        if (FUNDEF_ATTRIB (fundef) == ST_call_mt_master) {
            ...
            /*
                allocate  = MakeIcm2( "MT2_ALLOCATE",
                                      ...
                                      BuildParamsByDFM( ST_ALLOC( arg_node), "alloc",
               NULL, NULL));
            */
            broadcast = MakeIcm4 ("MT2_MASTER_BROADCAST", MakeId1 ("SYNC"),
                                  MakeNum (MT_IDENTIFIER (arg_node)),
            ...
            receive = MakeIcm4 ("MT2_MASTER_RECEIVE", MakeId1 ("SYNC"),
                                MakeNum (MT_IDENTIFIER (arg_node)),
                                MakeNum (DFMTestMask (bset)),
                                BuildParamsByDFM (bset, "bset", NULL, NULL));
...
,
    ...
    node *COMPMTalloc (node * arg_node, node * arg_info)
    {
        ...
        receive
          = MakeIcm3 (receive_icm, MakeId1 ("ALLOC"),
                      MakeNum (DFMTestMask (MTALLOC_IDSET (arg_node))),
                      BuildParamsByDFM (MTALLOC_IDSET (arg_node), "alloc", NULL, NULL));
    ...
    node *COMPMTsync (node * arg_node, node * arg_info)
    {
        ...
        args = BuildParamsByDFMfold (MTSYNC_FOLD (arg_node), "sync", &nums,
                                     BuildParamsByDFM (MTSYNC_ALLOC (arg_node), "sync",
                                                       &nums, NULL));
        ...
        result = MakeAssigns9 (if_, alloc, wait, broadcast, activate, else_, suspend,
                               receive, end_);
