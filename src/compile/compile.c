...
COMPIdxModarray (node *arg_node, node *arg_info)
{
    ...
    /* index is a variable or a N_prf generated by IVE! */
    DBUG_ASSERT (((NODE_TYPE (arg2) == N_prf) || (NODE_TYPE (arg2) == N_id)),
                 "wrong 2nd arg of modarray");

    if (NODE_TYPE (arg3) == N_array) { /* value is constant! */
        DBUG_ASSERT (0, "sorry compilation of ND_IDX_MODARRAY_AxVxC not yet done");
    } else {
        DBUG_ASSERT (((NODE_TYPE (arg2) == N_prf)
                      || ((TYPES_DIM (ID_TYPE (arg2)) == 0)
                          && (TYPES_BASETYPE (ID_TYPE (arg2)) == T_int))),
                     "indexing var of wrong type as 2nd arg of modarray!");
...
{
    node *icm_args, *neutral, *info, *assigns = NULL;
    ids *withid_ids;
    char *icm_name;
...
COMPWLgrid (node *arg_node, node *arg_info)
{
    ...
    if (WLGRID_NEXTDIM (arg_node) != NULL) {
        ...
        if (WLGRID_CODE_TEMPLATE (arg_node) == 0) {
            ...
            /*
             * proper code found (it is not just a template).
             */
            ...
            case WO_foldprf:
                ...
                /*
                 * insert the code of the dummy fold-fun
                 */
#if 0
          assigns = AppendAssign( assigns, ...);
#endif
            ...
            /*
             * code is just a template (otherwise: init/copy/noop).
             */
            ...
            case WO_foldprf:
                /*
                 * first argument of 'icm_args2' is not needed
                 */
                icm_args2 = FreeNode (icm_args2);
    ...
    if ((IDS_REFCNT (NWITHID_VEC (NWITH2_WITHID (wl_node))) >= 0)
        && (WLGRID_CODE_TEMPLATE (arg_node) == 0)) {
        /*
