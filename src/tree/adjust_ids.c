...
node *RenameIds( node *ids_chain)
{
  ...
  while (tmp != NULL) {
    ...
    ILIBfree( IDS_NAME( tmp));
    IDS_NAME( tmp) = ILIBstringCopy( VARDEC_OR_ARG_NAME( IDS_VARDEC( tmp)));
...
node *RenameId( node *id_node)
{
  ...
  ILIBfree( ID_NAME( id_node));
  ID_NAME( id_node) = ILIBstringCopy( VARDEC_OR_ARG_NAME( ID_VARDEC( id_node)));
...
node *FindOrMakeVardec(char *var_name, node *fundef, node *vardec_or_arg) 
{
  ...
  if (vardec == NULL) {
    if (NODE_TYPE(vardec_or_arg) == N_vardec) {
      ...
      vardec = MakeVardec(ILIBstringCopy(ARG_NAME(vardec_or_arg)),
 ...
 *
 * function:
 *   node *AIids(node *arg_ids, info *arg_info)
 *
 * description:
 *   
 *
...
node *AIids(node *arg_ids, info *arg_info)
{
  ...
  if (0 != strcmp(IDS_NAME(arg_ids), IDS_NAME(INFO_AI_IDS(arg_info)))) {
    new_let = MakeLet(MakeId(ILIBstringCopy(IDS_NAME(INFO_AI_IDS(arg_info))),
                             NULL,
                             ST_regular),
                      arg_ids);
#if 0
    ...
    INFO_AI_POSTASSIGN(arg_info) = MakeAssign(new_let,
                                              INFO_AI_POSTASSIGN(arg_info));
    arg_ids = MakeIds(ILIBstringCopy(IDS_NAME(INFO_AI_IDS(arg_info))),
                      NULL,
                      ST_regular);
...
node *AIid( node *arg_node, info *arg_info)
{
  ...
  if (INFO_AI_IDS(arg_info) != NULL) {
    ...
    if (0 != strcmp(ID_NAME(arg_node), IDS_NAME(INFO_AI_IDS(arg_info)))) {
      ...
      INFO_AI_PREASSIGN( arg_info) = MakeAssign( new_let,
  ...
  if (INFO_AI_ARGS(arg_info) != NULL) {
    ...
    if (0 != strcmp(ID_NAME(arg_node), desired_name)) {
      ...
      new_let = MakeLet(arg_node,
                        MakeIds(ILIBstringCopy(desired_name),
                                NULL,
                                ST_regular));
#if 0
      ...
      INFO_AI_PREASSIGN(arg_info) = MakeAssign(new_let,
                                               INFO_AI_PREASSIGN(arg_info));
      arg_node = MakeId(ILIBstringCopy(desired_name),
                        NULL,
                        ST_regular);
...
node *AIarg(node *arg_node, info *arg_info)
{
  ...
  ILIBfree( ARG_NAME(arg_node));
  ARG_NAME(arg_node) = ILIBstringCopy(ID_NAME(EXPRS_EXPR(INFO_AI_ARGS(arg_info))));
...
AdjustIdentifiers (node *fundef, node *let)
{
    ...
    DBUG_ASSERT ((FALSE), "Read comment in adjust_ids.c");

    /*
     * AdjustIdentifiers must be completely re-written:
     * 1. The implementation could be extremely streamlined because all identifiers
     *    are now concentrated in AVIS nodes.
     * 2. The call signature with a let-node is no longer feasible because in the
     *    only current application in precompile there is no natural let-node.
     *    Instead it is constructed for the only purpose of calling this function.
     *    Of course, there are no AVIS nodes in this construction and therefore
     *    the procedure is no longer feasible.
     */
